    ...
    while True:
        try:
            ...
            for r in rows:
                ...
                event_time = datetime.strptime(start_time, "%Y-%m-%d %H:%M:%S")
...
def events(today: bool = False):
    ...
    if today:
        ...
        rows = fetch(f"""
            ...
            FROM events
            WHERE start_time >= '{today_str} 00:00:00'
              AND start_time <  '{today_str} 23:59:59'
            ORDER BY start_time
        """)
    else:
        rows = fetch("""
...
def events_music():
    rows = fetch("""
        SELECT id, category, title, description, location, price, start_time, end_time
        FROM events
        WHERE category = 'music' or category = 'Music'
        ORDER BY start_time
    """)
...
# ---------------- ALERTS ----------------
@app.get("/api/alerts")
def get_alerts(endpoint: str = None):
...
async def subscribe(request: Request):
    ...
    # 🔴 REPLACE ENTIRE subscribe() INSERT BLOCK
