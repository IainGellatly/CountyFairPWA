...
function renderAlertButton(e){
  const now = Date.now();
  const startTime = parseDateTime(e.start_time);
  ...
  return `
    <button class="alert-btn" data-event-id="${e.id}">
      🔔 Alert Me
    </button>
  `;
}
...
async function loadEvents(){
  ...
  if (endpoint){
    [eventsRes, alertsRes] = await Promise.all([
      fetch('/api/events'),
      fetch(`/api/alerts?endpoint=${encodeURIComponent(endpoint)}`)
    ]);
  } else {
    [eventsRes, alertsRes] = await Promise.all([
      fetch('/api/events'),
  ...
  if (!notificationsEnabled()) {
    ...
    if (isIOS()) {
      ...
      if (!isStandalone()) {
        instructions = `
          📱 <b>iPhone Setup Required</b><br><br>
          1. Tap <b>Share</b> (⬆️)<br>
          2. Tap <b>Add to Home Screen</b><br>
          3. Open app from home screen<br>
          4. Then enable notifications<br><br>
        `;
      } else {
        instructions = `
          📱 <b>Enable Notifications</b><br><br>
          Tap below and allow notifications<br><br>
        `;
      }
      ...
      instructions = `
        🔔 <b>Enable Notifications</b><br><br>
        Tap below to allow notifications<br><br>
      `;
    }

    h += `
      <div class="card ${isIOS() ? 'ios-note' : ''}" id="notifyCard">
...
function installApp(){
  ...
  // iPhone NOT installed → block + guide
  if (isIOS() && !isStandalone()){
    alert("On iPhone, you must 'Add to Home Screen' first.\n\nTap Share → Add to Home Screen, then open the app and enable notifications.");
    return;
  }
...
async function registerPush(){
 ...
 if (permission !== 'granted'){
  alert("Notifications were blocked. You can enable them later in browser settings.");
  return;
}
...
function isEventActive(start, end){
  const now = Date.now();
  return now >= parseDateTime(start) && now <= parseDateTime(end);
}
...
async function loadFood(){
  ...
  d.forEach(i => {
    h += `<div class="card"><b>${i.name}</b><br>${renderLine(i.description)}<br>${renderLine(i.location)}</div>`;
  });
...
async function loadMusic(){
  ...
  let h = '<h2>Music</h2>';
  d.forEach(i => {
    h += `<div class="card"><b>${i.name}</b><br>${renderLine(i.description)}<br>${renderLine(i.location)}<br>${i.datetime}</div>`;
  });
...
async function loadDynamic(type){
  ...
  data.forEach(i => {
    h += `
      <div class="card">
        <b>${i.name}</b><br>
        ${renderLine(i.description)}<br>
        ${renderLine(i.location)}
      </div>
    `;
  });
...
async function loadTodayEvents(){
  ...
  events.forEach(e => {
    ...
    const activeClass = isEventActive(e.start_time, e.end_time)
      ? "active-event"
      : "";
